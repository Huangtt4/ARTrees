...
public class FutureTreeSpawn : MonoBehaviour
{
	[SerializeField]
	AbstractMap _map;
	...
	private void PrepareJSON()
	{
		...
		int size = (TreeParser.getFutureTrees() != null)? TreeParser.getFutureTrees().Count:0;
		for (int i = 0; i < size; i++)
		{
			string location = TreeParser.getFutureTrees()[i].CoordinatesY + "," + TreeParser.getFutureTrees()[i].CoordinatesX;
	...
	private void UpdateObjects()
	{
		...
		for (int i = 0; i < _locationStrings.Count; i++)
		{
			...
			treeObject.setAttribute(TreeParser.getFutureTrees()[i]);
