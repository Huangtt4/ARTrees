...
public class RemovedTreeSpawn : MonoBehaviour
{
	[SerializeField]
	AbstractMap _map;
	...
	private void PrepareJSON()
	{
		...
		int size = (TreeParser.getRemovedTrees() != null) ? TreeParser.getRemovedTrees().Count : 0;
		for (int i = 0; i < size; i++)
		{
			string location = TreeParser.getRemovedTrees()[i].CoordinatesY + "," + TreeParser.getRemovedTrees()[i].CoordinatesX;
	...
	private void UpdateObjects()
	{
		...
		for (int i = 0; i < _locationStrings.Count; i++)
		{
			...
			treeObject.setAttribute(TreeParser.getRemovedTrees()[i]);
